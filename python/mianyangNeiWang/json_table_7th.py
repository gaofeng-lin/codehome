...
# 这个路径存放的是cfd_para.hypara版本文件
init_path='F:/lgf/name_split2/ActiveBranch'
file_name_list=re.split(r'[/]',str(init_path))
# 这个file_name是从上面init_path路径提取出来的，取最后一个。这里是try1
...
def init_table(init_path,result_path):
    ...
    # print(dict)
    if os.path.getsize(result_path+file_name+'.json')==0: 
        with open(result_path+file_name+'.json','w') as f:
            json.dump(dict,f,sort_keys=True,indent=4,ensure_ascii=False)
        f.close()

        f=open(result_path+'manual_check'+'.json','w')
        f.close()
    else:
        return 
...
def manual_check(new_version_num,list,result_path):

    # 如果这个文件里面什么内容都没有，先初始化
    if os.path.getsize(result_path+'manual_check'+'.json')==0:
        dict=init_manual_check_file(new_version_num,list)
        with open(result_path+'manual_check'+'.json','w') as f:
            json.dump(dict,f,sort_keys=True,indent=4,ensure_ascii=False)
            f.close()
        ...
        for j in list:
            ...
            f=open(result_path+'manual_check'+'.json','r')
...
def from_manual_check_to_paracmp(result_path):
...
def add_value_firclass(new_version_num,new_value):
    ...
    if isinstance(new_value,list):
        for i in new_value:
            ...
            f=open(result_path+file_name+'.json','w')
...
def add_value_secclass(old_value,new_value,new_version_num):
    f=open(result_path+file_name+'.json','r')
    new_data={'name':new_value,'version':new_version_num}
    ...
    if old_value in old_data:
        ...
        for i in old_data:
            if isinstance(old_data[i],list):
                for j in old_data[i]:
                    if old_value in j.values():
                        new_data1={'name':new_value,'version':new_version_num}
...
# 先读取manual_check,找到最大的version。然后和file_list里面的版本号进行对比，从第一个比它大的开始。
# 例子：我之前更新到了8000，但是现在需要更新到9000。列表里面只能放8000-9000的。如果放了之前的版本，会覆盖之前的，需要重头来
def ensure_ver_num(manual_check_path,file_list):
    list_tmp=[]
    tmp=0
    if os.path.getsize(manual_check_path)==0:
        return 
    else:
        f=open(manual_check_path,'r')
        line=f.readline()
        while line:
            if 'version' in line:
                new_line=re.split(r'[\t,\n,:]',str(line))
                # print(new_line)
                # print(int(new_line[1].strip()))
                str_tmp=eval(new_line[1].strip())
                if(int(str_tmp)>tmp):
                    tmp=int(str_tmp)
            line=f.readline()
...
# 迭代比较整个文件夹里面的文件
def comapre_folder(file_path,result_path):
    file_one=os.listdir(file_path)
    if len(file_one)==1:
        return
    
    # 将文件夹里面的文件按照版本号从低到高排列
    file_list=sort_version_list(file_path)

    # 这个值记录的是manual_check里面最大的版本号
    tmp_num=ensure_ver_num(result_path+'manual_check'+'.json',file_list)
    tmp_index=0
    if tmp_num!=None:
        for i in range(len(file_list)):
            ver_tmp=re.sub('\D','',file_list[i])
            if tmp_num<int(ver_tmp):
                tmp_index=i
                break
        # print(type(ver_tmp))
    start_num=0
    if tmp_index!=0:
        start_num=tmp_index-1

    for i in range(start_num,len(file_list)):

        # 判断manual_check文件中是否存在 false字符，存在则程序停止;如果不存在则执行函数from_manual_check_to_paracmp
        flag=check_file(result_path+'manual_check'+'.json')
        if flag == 'manual_check':
            ...
            if con=='ok':
                from_manual_check_to_paracmp(result_path)
        ...
        if len(l1)==2:
            ...
            if len(set(new_have_zero))!=1 and len(set(old_have_zero))!=1:
                ...
                if len(set(manual_check_list))==1:
                    ...
                    manual_check(new_version_num,manual_check_list,result_path)
...
if __name__ == '__main__':
    ...
    init_table(init_path,result_path)

    comapre_folder(init_path,result_path)
