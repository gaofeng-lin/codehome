import h5py
import numpy as np
# 这个代码是从64*64的h5文中选取满足控制方程的点，把这些点单独生成h5文件
...
X_64, y_64 = get_65_h5file()


combined = y_64['data'].reshape((101, 64, 64, 3))
...
for frame_index in range(101):
    for i in range(1, 63):  # Skipping the first and last points in space
        for j in range(1, 63):
            # idx = frame_index * 64 * 64 + i * 64 + j
            # x, y = -np.pi + i*h, -np.pi + j*h
            # 获取 ux, uy, p 的值
            ...
            if MSE_total < 4e-5:
                ...
                count = count + 1
                XYT_selected.append([x, y, t])
                UX_selected.append(ux)
                UY_selected.append(uy)
                P_selected.append(p)
...
XYT_selected = np.array(XYT_selected)
UX_selected = np.array(UX_selected)
UY_selected = np.array(UY_selected)
P_selected = np.array(P_selected)

with h5py.File('C:\\Users\\76585\\Desktop\\select_point\\select_64_point.h5', 'w') as f_new:
    for unique_time in np.unique(XYT_selected[:, 2]):
        mask_time = XYT_selected[:, 2] == unique_time
        dset = f_new.create_dataset(str(unique_time), (mask_time.sum(), 5), dtype='f')
        dset[:, 0:2] = XYT_selected[mask_time, 0:2]
        dset[:, 2] = UX_selected[mask_time]
        dset[:, 3] = UY_selected[mask_time]
        dset[:, 4] = P_selected[mask_time]
