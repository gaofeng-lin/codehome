...
# 找到本次循环中的最大值和对应的索引
local_max_idx = np.argmax(z)
local_max = z[local_max_idx]

# 如果本次循环的最大值大于当前最大值，则更新最大值和最大值索引
if local_max > zmax:
    zmax = local_max
    zmax_idx = (x[local_max_idx], y[local_max_idx], zmax)
...
# 绘制最大值点，用红色表示
ax.scatter(zmax_idx[0], zmax_idx[1], zmax_idx[2], color='red', s=100)
