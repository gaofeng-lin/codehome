...
def get_16_h5file():
    f = h5py.File('C:\\Users\\76585\\Desktop\\select_point\\data_16.h5', 'r')
    n_frame = len(f.keys())
    XYT = np.zeros((0,3))
    UX = np.zeros((0,1))
    UY = np.zeros((0,1))
    ...
    for i in range(1, 63):  # Skipping the first and last points in space
        for j in range(1, 63):
            ...
            if MSE_total < 4e-5:
                ...
                count = count + 1
                new_XYT = np.array([[x, y, t]])
                new_UX = np.array([[ux]])
                new_UY = np.array([[uy]])
                new_P = np.array([[p]])

                XYT = np.concatenate((XYT, new_XYT), axis=0)
                UX = np.concatenate((UX, new_UX), axis=0)
                UY = np.concatenate((UY, new_UY), axis=0)
                P = np.concatenate((P, new_P), axis=0)
# print(MSE_total[4,,54])
print("count is : ", count)

# ... [previous code]

with h5py.File('C:\\Users\\76585\\Desktop\\select_point\\Updated_Prediction_store_16_64.h5', 'w') as f_out:
    unique_times = np.unique(XYT[:, 2])
    for time in unique_times:
        indices = np.where(XYT[:, 2] == time)
        combined_data = np.concatenate((XYT[indices][:, :2], 
                                        UX[indices], 
                                        UY[indices], 
                                        P[indices]), axis=1)
        f_out.create_dataset(str(time), data=combined_data)
